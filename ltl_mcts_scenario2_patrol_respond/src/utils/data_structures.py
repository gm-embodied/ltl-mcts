...
Include agent state, target state, world state, observation data, etc.
"""
...
class Measurement:
    ...
    def __init__(self, value: np.ndarray, is_clutter: bool = False, 
        ...
        self.is_clutter = is_clutter                 # Whether clutter
        self.source_target_id = source_target_id     # Source target ID (if not clutter)

class Observation:
    """All observations in a time step"""
    ...
    def get_measurement_array(self) -> np.ndarray:
        """Return all measurement points' numpy array, shape (n_measurements, 2)"""
...
class BernoulliComponent:
    """Bernoulli component - represents a tracked target's hypothesis"""
    ...
    def get_covariance_trace(self) -> float:
        """Get position covariance matrix's trace"""
        return np.trace(self.get_position_cov())
    
    def is_confirmed(self, threshold: float) -> bool:
        """Check if target is confirmed (covariance trace less than threshold)"""
...
class PoissonComponent:
    ...
    def get_expected_number_in_region(self, region) -> float:
        """Calculate the expected number of targets in the given region
        Simplified: if intensity function is uniform λ(x)=λ0, then E[#|R]≈λ0 * Area(R)
        """
        try:
...
class GlobalHypothesis:
    ...
    def get_map_estimate(self) -> List[BernoulliComponent]:
        """Get maximum a posteriori estimate of target list"""
...
class PMBMBelief:
    ...
    def get_map_hypothesis(self) -> Optional[GlobalHypothesis]:
        """Get maximum a posteriori global hypothesis"""
    ...
    def get_top_k_hypotheses(self, k: int = 2) -> List[GlobalHypothesis]:
        """Get top k hypotheses by weight"""
    ...
    def get_weight_gap(self) -> float:
        """Get weight difference between top two hypotheses"""
...
class AtomicProposition:
    """Atomic proposition"""
    name: str                    # Proposition name, e.g. "ap_confirm_any"
...
class MCTSNode:
    ...
    def get_best_child(self) -> Tuple[int, 'MCTSNode']:
        """Get child node with most visits"""
...
def create_initial_pmbm_belief() -> PMBMBelief:
    ...
    belief = PMBMBelief()
    # Use PPP density from config, avoid region prior existence probability too strong
    try:
    ...
    belief.global_hypotheses = [GlobalHypothesis(1.0, [])]  # Initial hypothesis: no target
    return belief
