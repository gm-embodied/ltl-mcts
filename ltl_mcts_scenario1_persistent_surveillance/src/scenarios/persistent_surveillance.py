...
Atomic proposition design:
- ap_exist_A: target exists in region A (probability > threshold)
- ap_exist_B: target exists in region B (probability > threshold)
...
class PersistentSurveillanceEnvironment:
    ...
    def initialize(self) -> Tuple[WorldState, PMBMBelief, LTLState]:
        ...
        # LTL state flag (restore original design)
    ...
    def step(self, action: int, planner_name: str = "Unknown") -> Tuple[WorldState, Observation, PMBMBelief, LTLState, bool, Dict[str, Any]]:
        ...
        # 5. Update atomic proposition state (restore original version)
        ...
        if current_step <= 5 or DEBUG_BRIDGE:
            self.logger.info(f"[CHECK] k={current_step} pA={pA:.3f} pB={pB:.3f} "
                           f"ws_on(A/B)={self.ws.apA_on}/{self.ws.apB_on} tA/tB={self.ws.t_A}/{self.ws.t_B} violate={violate}")
        
        # Debug output - print LTL status every 10 steps
        elif current_step % 10 == 0:
            self.logger.info(f"[LTL] step={current_step} pA={pA:.3f} pB={pB:.3f} "
        ...
        # Sync ap state to LTL memory (for planner to read)
        ...
        if violate:
            ...
            if is_done and not violate:
                # True LTL success condition:
                # 1. Both regions must be armed (i.e., detected targets)
                # 2. Cannot violate time window constraint
                both_regions_armed = self.ws.armed_A and self.ws.armed_B
                task_completed = both_regions_armed
                
                # Debug log
                if current_step <= 5 or is_done:
                    self.logger.info(f"[SUCCESS_CHECK] armed_A={self.ws.armed_A}, armed_B={self.ws.armed_B}, "
                                   f"both_armed={both_regions_armed}, violate={violate}")
            else:
                task_completed = False
